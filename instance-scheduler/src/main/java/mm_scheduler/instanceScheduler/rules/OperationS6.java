 ...
 *
 */
public class OperationS6 extends OperationRule implements Comparator<Operation>, Serializable {
	public OperationS6() {
		super(28, "S6");
	...
	public int compare(Operation operTask1, Operation operTask2) {
		...
		double s1 = (operTask1.getPrepOp() == null ? operTask1.getPart().getDueDate()
				: (operTask1.getPart().getDueDate() - operTask1.getPrepOp().getFinish())) / operTask1.getRemainOpNum();
		double s2 = (operTask2.getPrepOp() == null ? operTask2.getPart().getDueDate()
				: (operTask2.getPart().getDueDate() - operTask2.getPrepOp().getFinish())) / operTask2.getRemainOpNum();
		try {
			if (s1 > s2)
				i = 1;
			else if (s1 == s2) {
